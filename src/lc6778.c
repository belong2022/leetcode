...
int swimInWater(int** heights, int heightsSize, int* heightsColSize) {
    int m = heightsSize;
    int n = heightsColSize[0];
    int left = heights[0][0];
    int right = m * n - 1;
    int ans = 0;
    while (left <= right) {
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            if ( i + 1 < m) {
                ...
                edges[edgesSize].dh = fmax(heights[i][j], heights[i+1][j]);
                edgesSize++;
            }
            if (j + 1 < n) {
                edges[edgesSize].ps = i * n + j;
                edges[edgesSize].pe = i * n + j + 1;
                edges[edgesSize].dh = fmax(heights[i][j], heights[i][j+1]);
                edgesSize++;
            }
        }
    }
...
bool dfs(int **heights, int x, int y, int m, int n, int k, int* visit) {
    ...
    for (int i=0; i<4; ++i) {
        int x1 = x + diff[i][0];
        int y1 = y + diff[i][1];
        if (x1 >= 0 && x1 < m && y1 >= 0 && y1 < n && heights[x1][y1] <= k) {
